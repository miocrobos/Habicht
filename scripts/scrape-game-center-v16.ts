/**
 * SWISS VOLLEY LEAGUE SCRAPER v16 - WITH TEAMS EXPANSION
 * Clicks "Teams einblenden" if teams are collapsed
 */
...
async function main() {
  console.log('SWISS VOLLEY COMPLETE LEAGUE SCRAPER v16');
  console.log('=========================================\n');
  ...
  const expandTeamsIfNeeded = async () => {
    // Check if teams section is collapsed
    const einblenden = page.locator('text="Teams einblenden"').first();
    if (await einblenden.count() > 0) {
      console.log('  Teams collapsed - clicking to expand...');
  ...
  const scrapeTeams = async (key: keyof Omit<ClubLeagues, 'name'>, label: string) => {
    ...
    // Expand teams if collapsed
    await expandTeamsIfNeeded();
