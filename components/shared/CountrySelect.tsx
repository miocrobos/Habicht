...
export default function CountrySelect({ value, onChange, className, placeholder = "Select Country" }: CountrySelectProps) {
  ...
  return (
    <select
      ...
      className={className || 'w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-gray-700 dark:text-white'}
    >
      {!value && <option value="">{placeholder}</option>}
      {countries.map(country => (
        <option key={country.code} value={country.name} className="dark:bg-gray-700 dark:text-white">
